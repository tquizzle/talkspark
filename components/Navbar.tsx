    ...
    <div className="hidden md:flex fixed top-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-5xl px-4">
      ...
      `}>
        ...
        <Link to="/" className="flex items-center gap-3 group pl-2 pr-6">
          <div className="relative flex items-center justify-center w-11 h-11 rounded-full bg-gradient-to-tr from-indigo-500 via-purple-500 to-pink-500 text-white shadow-lg shadow-indigo-500/40 group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 ring-2 ring-white/20">
        ...
        <div className="flex items-center p-1.5 bg-black/20 dark:bg-slate-200/50 rounded-full border border-white/5 dark:border-black/5 backdrop-blur-sm">
          <Link to="/" className={`px-5 py-2 rounded-full text-sm font-bold transition-all duration-300 ${isActive('/') ? 'bg-slate-800 text-white dark:bg-white dark:text-black' : 'text-slate-400 hover:text-white dark:text-slate-500 dark:hover:text-black'}`}>Explore</Link>
          <Link to="/random" className={`px-5 py-2 rounded-full text-sm font-bold transition-all duration-300 ${isActive('/random') ? 'bg-slate-800 text-white dark:bg-white dark:text-black' : 'text-slate-400 hover:text-white dark:text-slate-500 dark:hover:text-black'}`}>Random</Link>
          <Link to="/submit" className={`px-5 py-2 rounded-full text-sm font-bold transition-all duration-300 ${isActive('/submit') ? 'bg-slate-800 text-white dark:bg-white dark:text-black' : 'text-slate-400 hover:text-white dark:text-slate-500 dark:hover:text-black'}`}>Submit</Link>
        </div>

        <div className="flex items-center gap-1 pl-4">
           <button 
             onClick={handleAdminClick} 
             className={`relative p-2 transition-colors ${isAuthenticated ? 'text-primary' : 'text-slate-400 hover:text-white dark:text-slate-500 dark:hover:text-black'}`} 
             title={isAuthenticated ? "Admin Panel" : "Admin Login"}
           >
             <Icon name={isAuthenticated ? "Target" : "Monitor"} size={20} />
           ...
           <div className="relative" ref={themeMenuRef}>
              <button onClick={() => setIsThemeMenuOpen(!isThemeMenuOpen)} className="w-10 h-10 flex items-center justify-center rounded-full text-slate-400 hover:bg-white/10 dark:text-slate-500 transition-all">
                <Icon name={themes.find(t => t.id === theme)?.icon || 'Sun'} size={20} />
              </button>
              {isThemeMenuOpen && (
  ...
  const MobileBottomNav = () => (
    <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] z-50">
      <div className="bg-slate-900/90 dark:bg-white/90 backdrop-blur-xl border border-white/10 dark:border-black/5 shadow-2xl rounded-full flex justify-around items-center px-4 py-4">
        ...
        <button onClick={handleAdminClick} className={`flex flex-col items-center justify-center ${isActive('/admin') ? 'text-white dark:text-black' : 'text-slate-400'}`}>
          <Icon name={isAuthenticated ? "Target" : "Monitor"} size={24} />
...
const MobileNavItem: React.FC<{ to: string; icon: string; label: string; active: boolean }> = ({ to, icon, label, active }) => (
  <Link to={to} className={`flex flex-col items-center justify-center transition-all ${active ? 'text-white dark:text-black scale-110' : 'text-slate-400 dark:text-slate-500'}`}>
